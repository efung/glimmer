...
public class GlimmerLiveWallpaper extends BaseLiveWallpaperService implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    private boolean readSettingsFromPreferences() {
        ...
        temp = Integer.valueOf(prefs.getString(this.getString(R.string.prefs_key_mode), String.valueOf(GlimmerPreferenceActivity.PREFS_MODE_DEFAULT)));
        if (this.mMode != temp) {
            this.mMode = temp;
            settingsChanged = true;
        }

        temp = Integer.valueOf(prefs.getString(this.getString(R.string.prefs_key_dot_size), String.valueOf(GlimmerPreferenceActivity.PREFS_DOT_SIZE_DEFAULT)));
        ...
        temp = prefs.getInt(this.getString(R.string.prefs_key_brightness), 50);
        if (this.mBrightness != temp) {
            this.mBrightness = temp;
            settingsChanged = true;
        }
    ...
    private void loadParticleImage() {
        this.mParticleTextureRegion = BitmapTextureAtlasTextureRegionFactory.createFromAsset(this.mBitmapTextureAtlas, this,
                getParticleFilename(this.mDotSize), 0, 0);
    ...
    private void buildReflectLightScene(final Scene scene) {
        ...
        this.mParticleSystem = new BatchedSpriteParticleSystem(particleEmitter, maxParticles, maxParticles, maxParticles + maxParticles / 4 /*
                                                                                                                                             * A little extra to
                                                                                                                                             * ensure coverage
                                                                                                                                             */,
    ...
    private void buildColourChangeScene(final Scene scene) {
        final GridParticleEmitter particleEmitter = new GridParticleEmitter(CAMERA_WIDTH * 0.5f, CAMERA_HEIGHT * 0.5f, CAMERA_WIDTH, CAMERA_HEIGHT,
                this.mParticleTextureRegion.getWidth(), this.mParticleTextureRegion.getHeight(), false);
        final int maxParticles = particleEmitter.getGridTilesX() * particleEmitter.getGridTilesY();
        this.mParticleSystem = new BatchedSpriteParticleSystem(particleEmitter, maxParticles / PARTICLE_LIFETIME, maxParticles / PARTICLE_LIFETIME,
                maxParticles, this.mParticleTextureRegion, this.getVertexBufferObjectManager());
        ...
        this.mEngine.registerUpdateHandler(new TimerHandler(mColourChangePeriod, true, new ChangingColorParticleInitializerTimerHandler(initialColor,
    ...
    private class ChangingColorParticleInitializerTimerHandler implements ITimerCallback {
        ...
        public ChangingColorParticleInitializerTimerHandler(final Color pColor, final MutableColorParticleInitializer<UncoloredSprite> pInitializer) {
        ...
        public void onTimePassed(TimerHandler pTimerHandler) {
            ...
            hsv[2] = ((float) mBrightness) / 100.0f;
